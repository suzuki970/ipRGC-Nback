...
class makeAuditoryStim:    
    ...
    def composeTones(self,repTimes):
        ...
        if os.name == 'nt': # if on windows
            fileList = [x.replace(os.sep,'/') for x in fileList]
    ...
    def makeStreamSound(self,fPath):
        
        fileList = glob.glob(fPath+'/*.wav')
        fileList.sort()
        
        filepath = fPath.split('/')[:-2]
        filepath = '/'.join(filepath) + '/stream'
        ...
        if self.sepFlg:
            out = wave.open(filepath + '/stereo_sep.wav','w')
        else:
            out = wave.open(filepath + '/stereo.wav','w')

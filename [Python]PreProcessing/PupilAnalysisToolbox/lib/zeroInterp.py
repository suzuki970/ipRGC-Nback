...
def zeroInterp_oneshot(pupilData):
    ...
    for iTrial,y_base in enumerate(list(pupilData)): 
        ...
        if len(zeroInd) > 0:
            ...
            #%%  find 0 values
            for i in np.arange(len(diffOnOff)-1):
                if diffOnOff[i] != 1 and diffOnOff[i+1] != 1:  #### one-shot noise
                    datOfblinkCood.append(np.array([zeroInd[i]-1,zeroInd[i]+1]))
                    count=count + 1
        
            #%%  
            for db in datOfblinkCood:
                y[db[0]+1] = np.mean([y[db[0]],y[db[1]]])
...
def zeroInterp(pupilData,fs,interval,windowL=0):
    ...
    for iTrial,y_base in enumerate(list(pupilData)): 
        ...
        if 0 in zeroInd:
            for iSample in np.arange(len(y)):
                if y[iSample] != 0 :
                    y[:iSample-1] = y[iSample]
                    y_base[:iSample-1] = y_base[iSample]
                    break
        ...
        if len(zeroInd) == len(y)-1:
            ...
            zeroArray.append(np.arange(len(y)))
            interpolatedArray.append(len(y))
            continue
        ...
        if len(zeroInd) > 0:
            ...
            if len(nonZero) != len(y):
                ...
                try:
                    yy = interpolate.PchipInterpolator(numX[nonZero], y[nonZero])
            ...
            zeroArray.append(np.argwhere(y == 0).reshape(-1))
        else:

...
#%%
def re_sampling_binary(dat,num):
    """
    Input:    
        dat  - list of data
    ...
    for iTrial,d in enumerate(dat):
        ...
        rep = math.ceil(1 / (len(d)/num - int(len(d)/num)))
        rep_all = []
        for i in np.arange(rep-1):
            rep_all = np.r_[rep_all, int(len(d)/num)]
        
        rep_all = np.r_[rep_all, int(len(d)/num+1), int(len(d)/num+2)]
        # rep_all = np.r_[rep_all, 11]
        rep_all = np.tile(rep_all,int(num/3))
        for i in np.arange(1,len(rep_all)):
            rep_all[i]+=rep_all[i-1]
...
def pre_processing(dat,cfg,rejectFlg=False):
    """
    Input:    
        dat  - 
        cfg  -
    ...
    if isinstance(dat, list):
        ...
        for i,p in enumerate(dat):
            ...
            baselineData = np.array([getNearestValue(x,wid_base[0][0]),getNearestValue(x,wid_base[0][1]),getNearestValue(x,wid_analysis[i])])
    ...
    #%% if data is array
        ...
        ## Smoothing
        if len(windowL) > 0:
            y = moving_avg(dat.copy(),int(np.array(windowL)))
        else:
            y = dat.copy()
        ## baseline(-200ms - 0ms)
        ...
        if len(wid_base) > 2:
            ...
            for iTrial in np.arange(len(wid_base)):
            ...
            baselineData = np.array([getNearestValue(x,wid_base[0][0]),getNearestValue(x,wid_base[0][1]),getNearestValue(x,wid_analysis)])
            baselinePLR = y[:,np.arange(baselineData[0],baselineData[1])]
            baselinePLR_std = np.std(baselinePLR,axis=1)
            baselinePLR_std = np.tile(baselinePLR_std, (1,y.shape[1])).reshape(y.shape[1],y.shape[0]).T

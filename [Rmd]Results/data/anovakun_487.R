...
# 31)anovakun version 4.8.6（R 4.1.0，R 4.1.1作成；2021.10.1公開）
# ・多重比較の出力時のヘッダを修正；mod.Bon.out関数の修正
# ・R 4.1.0以降での単純主効果の出力エラーを修正；anvoa.output関数の修正
# 32)anovakun version 4.8.7（R 4.2.0，R 2.1.1作成；2022.7.1公開）
# ・R 4.2.0以降での単純主効果の出力エラーを修正；simeffects.out関数の修正
#
# 【この関数の使用に関して】
...
anovakun <- function(dataset, design, ..., long = FALSE, type2 = FALSE, nopost = FALSE, tech = FALSE, data.frame = FALSE, copy = FALSE, 
	...
	info2 <- paste0("This output was generated by anovakun 4.8.7 under ", strsplit(R.version$version.string, " \\(")[[1]][1], ".")# バージョン情報など
...
boot.esci <- function(dat, design, factnames = NA, type2 = FALSE, nopost = FALSE, mainresults = NULL, postresults = NULL, 
	...
	if(.Platform$GUI == "Rgui"){# WindowsのRguiの場合（プログレスバーを表示すると10倍くらい時間がかかる）
		...
		cat(paste0("\n", round(as.numeric(Sys.time() - stm, units = "secs"), 2), " secs elapsed for bootstrapping.", "\n", sep = ""))
	}else{# その他のIDEの場合（プログレスバーを表示）
		cat("Now Bootstrapping...", "\n")
		bootresults <- do.call(cbind, lapply(1:B, function(v) {
			if(v %% (B/20) == 0) cat('\r', paste0("|", paste0(rep("=", ceiling(v/(B/50))), collapse = ""), paste0(rep(" ", 50 - ceiling(v/(B/50))), collapse = ""), "| ", 2 * v/(B/50), "%"))
			boot.anova(dat = bootdat[[v]], design = design, factnames = factnames, type2 = type2, dmat = dmat, flev = flev, 
				cellN = cellN, full.elem = full.elem, epsi.effect = epsi.effect, internal.lab = internal.lab, 
				int.sig.source = int.sig.source, sim.dmat = sim.dmat, sim.flev = sim.flev, sim.cellN = sim.cellN, 
				lb = lb, gg = gg, hf = hf, cm = cm, autov = autov, mau = mau, har = har, iga = iga, ciga = ciga, 
		...
		cat(paste0("\n", "Completed! ", round(as.numeric(Sys.time() - stm, units = "secs"), 2), " secs elapsed.", "\n", sep = ""))
	}
...
mod.Bon.out <- function(bon.list, omit = FALSE, sep = " "){
	...
	bonstat <- bon.list[[5]]
	bontab <- bon.list[[6]]
	Bon.header <- c("Pair", "Diff", "t-value", "df", "p", names(bontab[6]), "")
...
simeffects.out <- function(partresult, epsi.info2 = NA, epsi.header = NA, ano.info2 = NA, ano.header = NA, ano.nick = NA, 
	...
	cat(sprintf("%s", paste0("< SIMPLE EFFECTS for \"", part.info1, "\" INTERACTION >")), sep = "", "\n", "\n")# タイトル
	...
	# 球面性検定の結果を出力
	if(is.na(charmatch("Error", parttab$source.col)) && !is.null(nrow(partepsi))){
